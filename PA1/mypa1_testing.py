...
class MDP:
    ...
    - rewards (dict[str,float]): a dictionary of rewards by state names (Assume that an agent receive a state-specific reward when it reaches the corresponding state)
    ...
    def T(self, state: str, action: str) -> list:
        ...
        Returns:
            list[(str,float)]: a list of probabilities to all possible successor states e.g. [("s0", 0.1), ("s2", 0.3), ("s3", 0.6)]
        """
